from ai_agent import TankAgent
from game_loader import Game
import os

game = Game()
enemy_num = 1
state_size = 3*(enemy_num+1)
agent = TankAgent(state_size=state_size, action_size=5, epsilon=0.01)

# using the trained model to play the game
file_name = "tank_dqn.keras"

# you can also test the current training model which is not successfully trained because it is trained on 5 different levels
# file_name = "tank_dqn_training.keras"

if os.path.exists(file_name):
    agent.load(file_name)
    print("loaded model weights: " + file_name)

# current trained model should be able to play the level 1, 2, 3 with only 1 enemy
game.game_running_ai_play(agent, enemy_num = enemy_num, checkpoint = 3)